...
Send input to an AI model and receive a generated response. For a **live playground** with the correct body for a specific model, open that model in the [model catalog](/platform/model-catalog). The playground below shows a generic request shape only.
