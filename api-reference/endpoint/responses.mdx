...
Send input to an AI model and receive a generated response. For a **live playground** with the correct body for a specific model, open a [model page](/api-reference/models). The playground below shows a generic request shape only.
...
|-----------|------|----------|-------------|
| `model` | string | Yes | Model identifier. Open a [model page](/api-reference/models) for a dedicated playground with the correct body for that model. |
