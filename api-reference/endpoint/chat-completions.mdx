...
openapi: "openapi/zerogpu.openapi.yaml POST /chat/completions"
---

Some models are invoked with a **chat-completions** style body (`messages` array) instead of the [Responses](/api-reference/endpoint/responses) `input` field. The dashboard and [model catalog](/platform/model-catalog) indicate which route applies. Use the **API playground** below to try a live request.
