...
Some models are invoked with a **chat-completions** style body (`messages` array) instead of the [Responses](/api-reference/endpoint/responses) `input` field. The dashboard and [model catalog](/docs/model-catalog) indicate which route applies. Each model page has its own playground with the right examples for that model.
