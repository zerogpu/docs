...
| `url` | string | **Yes** | Must be `/v1/chat/completions`, the [only batchable endpoint](/api-reference/batch/supported-endpoints). **All lines in a single batch must share the same `url`**, and it must match the `endpoint` field on the create call. |
| `body` | object | **Yes** | The request body that would be sent to the synchronous endpoint. See [supported endpoints](/api-reference/batch/supported-endpoints)(/api-reference/batch/supported-endpoints) for the shape. The field `stream: true` is rejected. |
...
See [supported endpoints](/api-reference/batch/supported-endpoints)(/api-reference/batch/supported-endpoints) for the
...
See [supported endpoints](/api-reference/batch/supported-endpoints)(/api-reference/batch/supported-endpoints) for the exact
