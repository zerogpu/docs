...
| `url` | Yes | One of the [supported endpoints](/api-reference/batch/supported-endpoints). All lines in a batch must share the same `url`. |
...
<Note>
**ℹ Validation runs at create time**
...
The server streams the entire input JSONL and validates every line before
  ...
  get a 400 with the offending `line_idx`. Once the response
...
```json
{"custom_id": "q-1", "response": {"status_code": 200, "request_id": null, "body": { ... }}}
```

Error line shape (one per failed line):

```json
{"custom_id": "q-2", "error": {"code": "http_400", "message": "...", "details": { ... }}}
```
