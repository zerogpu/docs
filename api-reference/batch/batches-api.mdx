...
See [errors](/api-reference/batch/errors)(/api-reference/batch/errors) for the auth failure codes (`401`, `403`, `429`).
...
- `output_file_id` is set if at least one line returned 2xx. Download with
  ...
  [jsonl format](/api-reference/batch/jsonl-format)(/api-reference/batch/jsonl-format#output-jsonl) for the line schema.
- `error_file_id` is set if at least one line failed. Download with
  `GET /v1/files/{error_file_id}/content`. See
  [jsonl format](/api-reference/batch/jsonl-format)(/api-reference/batch/jsonl-format#error-jsonl) for the line schema.
