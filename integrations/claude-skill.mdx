...
Claude routes the request to ZeroGPU's PII model (`gliner2.5-multi-v1`) through the OpenAI-compatible API. If your API key isn't available yet, Claude asks for it first. Once authenticated, it makes the call and returns the model's actual output:
...
For multi-axis classification (for example sentiment **and** topic at once), Claude routes to `gliner2.5-base-v1` and returns one chosen label per axis:
...
| Multi-axis classify | `gliner2.5-base-v1` | "Classify by sentiment and topic with ZeroGPU: ..." |
...
| Detect PII | `gliner2.5-multi-v1` | "Detect PII in this text with ZeroGPU: ..." |
| Redact PII | `gliner2.5-multi-v1` | "Redact the PII in this with ZeroGPU: ..." |
| Extract entities | `gliner2.5-base-v1` | "Extract the people and organizations with ZeroGPU: ..." |
| Extract JSON | `gliner2.5-base-v1` | "Extract the name, email, and phone as JSON with ZeroGPU: ..." |
