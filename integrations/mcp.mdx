...
- **Backed by:** `zlm-v1-iab-classify-edge` (`zlm-v2-iab-classify-edge-enriched` when `enriched=true` or `multilingual=true`).
...
| `multilingual` | optional | `false` | Pass `true` when the text is not in English. Returns the same enriched shape. |
...
- **Backed by:** `gliner2.5-base-v1` (JSON mode).
...
| `model` | optional | `all-minilm-l6-v2` | Either `all-minilm-l6-v2` (22.7M, fastest) or `bge-small-en-v1.5` (33M, 512-token context, stronger on retrieval). |
