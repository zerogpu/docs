...
- **Backed by:** `llama-3.1-8b-instruct-fast` (`t5-small` when `compact=true`).

| Name | Required | Default | Description |
| --- | --- | --- | --- |
| `text` | yes | - | The passage to summarize (non-empty). |
| `max_tokens` | optional | model default | Upper bound on summary length, `1` to `2048`. |
| `compact` | optional | `false` | Route to `t5-small`, the cheapest summarizer. It reads only the first 512 tokens, so leave this off for longer passages. |
...
- **Backed by:** `zlm-v1-iab-classify-edge` (`zlm-v2-iab-classify-edge-enriched` when `enriched=true`, `zlm-v1-multi-iab-classify` when `multilingual=true`).
...
| `multilingual` | optional | `false` | Pass `true` when the text is not in English. Returns the same enriched shape, and wins over `enriched`, which is English-only. |
...
Classify a website into the IAB taxonomy from its domain name alone, with no page fetch: "what kind of site is espn.com?", bidstream and inventory enrichment, allow/deny list scoring. Use this when all you have is a hostname; when you have the page text, `zerogpu_classify_iab` is more precise per URL.

- **Backed by:** `zlm-v1-iab-domain-classifier`.

| Name | Required | Default | Description |
| --- | --- | --- | --- |
| `domain` | yes | - | Hostname, e.g. `nytimes.com`. A full URL is accepted and reduced to its hostname; the normalized value is echoed back on the result. |

Returns scored content categories across IAB 1.0 and 2.2, plus `topics`, `keywords`, and `user_intent`.

```json
{ "name": "zerogpu_classify_domain", "arguments": { "domain": "espn.com" } }
```
...
Check text against the 13 standard safety categories before it is published, stored, forwarded, or acted on: user-generated content, comments, support messages, prompts, ad creative, or anything from an untrusted source. Cheaper and more consistent than asking the host model to judge the content itself.

- **Backed by:** `zlm-v1-moderation-edge`, on the [Moderations](/api-reference/moderations) endpoint.

| Name | Required | Default | Description |
| --- | --- | --- | --- |
| `input` | yes | - | Text to classify. A single string, or an array of strings for one verdict per element. |

Returns `any_flagged` for a quick gate, a `results` array with one entry per input (each carrying `flagged`, the `flagged_categories` that tripped with their scores, and the full `category_scores` map), and the `model` used. This endpoint reports no token usage, so no `savings` block is returned.

```json
{ "name": "zerogpu_moderate", "arguments": { "input": "I will find you and make you regret this." } }
```

#### `zerogpu_embed`

Turn text into vectors for semantic search, RAG retrieval, clustering, deduplication, or similarity scoring.

- **Backed by:** `all-minilm-l6-v2` (`bge-small-en-v1.5` when `model` is set), on the [Embeddings](/api-reference/embeddings) endpoint.

| Name | Required | Default | Description |
| --- | --- | --- | --- |
| `input` | yes | - | Text to embed. A single string, or an array of strings for one vector per element. |
| `model` | optional | `all-minilm-l6-v2` | Either `all-minilm-l6-v2` (22.7M, fastest) or `bge-small-en-v1.5` (33.4M, 512-token context, stronger on retrieval). |

Returns an `embeddings` array (each entry an `index` and its `embedding`), the `count`, the vector `dimensions`, and `usage`. Both models return 384 dimensions, so they are interchangeable in an existing index, but don't mix them within one index. Embeddings bill on input tokens only.

```json
{ "name": "zerogpu_embed", "arguments": { "input": ["ZeroGPU runs inference at the edge.", "Frontier models are expensive."] } }
```
...
| `zerogpu_classify_domain` | IAB classification from a bare hostname | `domain` |
...
| `zerogpu_moderate` | Safety-check text across 13 categories | `input` |
| `zerogpu_embed` | Embed text as 384-dim vectors | `input` |
