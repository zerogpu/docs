---
title: "Hermes Agent"
description: "Run Hermes Agent on ZeroGPU: set it as the model provider and connect the ZeroGPU MCP server for thirteen ready-made tools."
icon: "robot"
---
...
The first prints thirteen tools. The second confirms the agent's own model is answering from ZeroGPU. Then ask for real work:
...
|---|---|---|---|---|
| `deepseek-v4-flash` | 1,048,576 | $0.16 | $0.38 | Default. Agentic work with tool calling. |
...
<CardGroup cols={2}>
	...
	<Card title="MCP server" icon="plug" href="/integrations/mcp">
		Full argument reference for all thirteen tools.
	</Card>
	<Card
		title="Cookbook"
		icon="book-open"
