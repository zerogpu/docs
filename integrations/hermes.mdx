...
hermes config set providers.ZeroGPU.default_model 'deepseek-v4.1-flash'
...
|---|---|---|---|---|
| `deepseek-v4.1-flash` | 1,048,576 | $0.30 | $1.20 | Default. Agentic work with tool calling. |
| `gpt-oss-120b` | 131,072 | $0.15 | $0.60 | Reasoning and function calling at mid cost. |
| `qwen3-30b-a3b-fp8` | 32,768 | $0.10 | $0.45 | Below the 64K minimum. Not usable as the agent model. |

Switch mid-session with `/model custom:ZeroGPU:gpt-oss-120b`.
...
| `zerogpu_summarize` | `llama-3.1-8b-instruct-fast` | `text` |
...
| `zerogpu_classify_structured` | `gliner2.5-base-v1` | `text`, `schema` |
| `zerogpu_extract_entities` | `gliner2.5-base-v1` | `text`, `labels` |
| `zerogpu_extract_json` | `gliner2.5-base-v1` | `text`, `schema` |
| `zerogpu_extract_pii` | `gliner2.5-multi-v1` | `text` |
| `zerogpu_redact_pii` | `gliner2.5-multi-v1` | `text` |
...
**Hermes rejects the model at startup.** The context window is under 64,000 tokens. Use `deepseek-v4.1-flash` or `gpt-oss-120b`, and set `context_length` explicitly if auto-detection fails.
