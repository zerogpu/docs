...
- A look at the [model catalog](/platform/model-catalog) if you want to see what each tool routes to.
...
**`ZeroGPU rate limit exceeded (429)`** - back off and retry with exponential delay, or move bulk workloads to the [Batch API](/api-reference/batch/index), which has separate quotas tuned for offline jobs.
