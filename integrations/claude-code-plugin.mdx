...
- A model ID from the [model catalog](/platform/model-catalog) if you plan to call models by name.
...
**Skill returns "You're not signed in yet."** - no credentials on disk. Run `/zerogpu-router:login` inside Claude Code, or `zerogpu login` in your terminal. Check `/zerogpu-router:status` to confirm.
...
**`Request failed with status 401`** - your API key is missing, revoked, or mistyped. Rotate the key in the [dashboard](https://platform.zerogpu.ai/dashboard) and re-run `/zerogpu-router:login`. Keys must start with `zgpu-api-`.
