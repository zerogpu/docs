...
For **large async jobs** (thousands of requests, 24-hour window, OpenAI-compatible JSONL), use the [Batch & Files API](/api-reference/batch) instead of firing parallel sync calls.

When you need **real-time** results for many items (e.g. summarize 100 articles today), send requests in parallel and handle errors so one failure doesn't block the rest.
