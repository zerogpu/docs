...
## 🏷️ Tag a Review with ZeroGPU

ZeroGPU is an ultra-fast, compute-efficient inference provider for apps and agents. We run purpose-built small and nano language models across an edge-powered network for the high-volume, purpose-specific tasks your app or agent runs constantly. Plug in our OpenAI-compatible API and you're live - zero GPU infrastructure, serverless, auto-scaling by default. In this section, we will tag a single review as an example, so you can see exactly what the model returns before pointing it at a whole file.

We pin the model to JSON-only output with `temperature: 0` so the parser does not have to be clever. The same system prompt drives every row of the batch later.
...
_This section takes a CSV of customer reviews and produces a tagged copy plus a recoverable list of failures, with the Batch API running every row as one asynchronous job while you sleep._
...
The job has six steps: prepare the CSV, build a JSONL request file, upload it, create the batch, poll until it finishes, then download and merge the results.

### Step 1: Prepare the input CSV

Place your export in the working directory. The recipe assumes a CSV with a stable, unique `review_id` and a free-text `review` column. Note that **row 7 is intentionally malformed** (missing the `review` value); you'll see how it is handled before it ever reaches the API.
...
Each line in the JSONL is an independent chat completion. `custom_id` is the only way to match a result back to a CSV row, because the Batch API does not preserve input order in the output file. Use your `review_id` directly so the join is unambiguous.
...
### Step 6: Recover the failed rows
