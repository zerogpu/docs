...
You'll need to set up your ZeroGPU API key so that every Batch API call authenticates. This ensures the upload, create, poll, and download calls can reach ZeroGPU securely. A Project ID is optional: supply one only if you want to scope the batch to a specific project.
...
# Prompt for the ZeroGPU API key securely
zerogpu_api_key = getpass('Enter your ZeroGPU API key: ')
os.environ["ZEROGPU_API_KEY"] = zerogpu_api_key

# Optional: press Enter to skip and run unscoped
zerogpu_project_id = getpass('Enter your ZeroGPU Project ID (optional): ')
if zerogpu_project_id:
...
export ZEROGPU_PROJECT_ID="your-project-uuid"   # optional
```
...
# x-project-id is optional; send it only when scoping to a project
if os.environ.get("ZEROGPU_PROJECT_ID"):
    headers["x-project-id"] = os.environ["ZEROGPU_PROJECT_ID"]
...
HEADERS = {"x-api-key": os.environ["ZEROGPU_API_KEY"]}
# x-project-id is optional; send it only when scoping to a project
if os.environ.get("ZEROGPU_PROJECT_ID"):
    HEADERS["x-project-id"] = os.environ["ZEROGPU_PROJECT_ID"]
