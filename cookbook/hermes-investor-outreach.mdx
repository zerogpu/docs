...
── Model provider → ZeroGPU ──
  ok  provider configured - model deepseek-v4-flash-0731
...
── Done ──
    Model    : deepseek-v4-flash-0731 via custom:ZeroGPU
```

The agent's own reasoning now runs on ZeroGPU's `deepseek-v4-flash-0731`, and fourteen ZeroGPU tools are available to it. For what the script does line by line, see the [Hermes Agent integration guide](/integrations/hermes).
...
Per-model subtotal
  ...
  deepseek-v4-flash-0731        5 calls    $0.00318
