...
── Model provider → ZeroGPU ──
  ok  provider configured - model deepseek-v4.1-flash
...
── Done ──
    Model    : deepseek-v4.1-flash via custom:ZeroGPU
```

The agent's own reasoning now runs on ZeroGPU's `deepseek-v4.1-flash`, and fourteen ZeroGPU tools are available to it. For what the script does line by line, see the [Hermes Agent integration guide](/integrations/hermes).
...
Per-model subtotal
  gliner2.5-base-v1              9 calls    $0.00019
  bge-small-en-v1.5            3 calls    $0.00047
  llama-3.1-8b-instruct-fast   5 calls    $0.00031
  deepseek-v4.1-flash          5 calls    $0.00318
