---
title: "Tutorial: Résumé & profile extraction"
description: "Step-by-step guide with example dataset—text-only extraction, no OCR."
---

This tutorial walks through **three production-style use cases** for the [Data extraction](/cookbook/data-extraction) recipe. You will use plain text only (résumés, LinkedIn-style exports, job posts)—**not** OCR or PDF parsing.

<Note>
All sample people and companies in the [example dataset](https://github.com/zerogpu/docs/tree/main/tutorials/data-extraction/dataset) are **synthetic and fictional**.
</Note>
...
| Step | Input | Output |
| --- | --- | --- |
| 1 | Plain-text résumé | `candidate` JSON (name, title, skills, …) |
...
Get your **API key** and **project ID** from the [dashboard](https://zerogpu.ai). See [Authentication](/platform/authentication).

## Before you start

Clone or browse the tutorial bundle in the docs repo:
...
└── blog-post.md      # Long-form article (publish-ready)
```

GitHub: [zerogpu/docs/tutorials/data-extraction](https://github.com/zerogpu/docs/tree/main/tutorials/data-extraction)

## Step 1 — Extract a candidate from a résumé
...
# First line of tutorials/data-extraction/dataset/resumes.jsonl (clone docs repo)
...
candidate = json.loads(body["output"][0]["content"][0]["text"])
print(json.dumps(candidate, indent=2))
```

</CodeGroup>

**Check your result:** You should see a `candidate` object with populated strings. If a field is missing from the source text, it may be empty—tighten field descriptions or add a second pass only where needed.
...
Batch all synthetic résumés or profiles locally:
...
python run_batch_extraction.py --dataset job_posts
```

Results are written to `tutorials/data-extraction/outputs/*.jsonl`. Inspect them before wiring into production ETL.

For many files at once, combine this with [Batch requests](/cookbook/batch-requests) patterns (parallelism, retries).

## Optional — PII on inbound text

Before storing user-submitted résumés, run [PII extraction](/cookbook/data-extraction#pii-extraction) with `gliner-multi-pii-v1` and `extract-pii` or `redact`.
...
- **Monitor:** Use [Logs](/platform/logs) and [Usage](/platform/usage-analytics) in the dashboard.

## Go deeper

<CardGroup cols={2}>
  <Card title="Data extraction recipe" icon="book" href="/cookbook/data-extraction">
    API reference-style examples for json, NER, and PII.
  </Card>
  <Card title="Example dataset (GitHub)" icon="github" href="https://github.com/zerogpu/docs/tree/main/tutorials/data-extraction/dataset">
    Synthetic résumés, profiles, and job posts (JSONL).
  </Card>
  <Card title="Blog post (markdown)" icon="newspaper" href="https://github.com/zerogpu/docs/blob/main/tutorials/data-extraction/blog-post.md">
    Publish-ready long-form walkthrough.
  </Card>
  <Card title="gliner2-base-v1 playground" icon="play" href="/models/gliner2-base-v1">
