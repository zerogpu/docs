---
title: "Tutorial: Resume & profile extraction"
description: "Step-by-step guide with example dataset. Text-only extraction, no OCR."
---

This tutorial walks through **three production-style use cases** for the [Data extraction](/cookbook/data-extraction) recipe. You will use plain text only (resumes, LinkedIn-style exports, job posts). **Not** OCR or PDF parsing.
...
Before storing user-submitted resumes, run [PII extraction](/cookbook/data-extraction#pii-extraction) with `gliner-multi-pii-v1` and `extract-pii` or `redact`.
