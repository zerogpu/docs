...
`ZeroGPUExtractEntitiesTool` runs custom-label named-entity recognition on `gliner2.5-base-v1`. You pass the entity types you care about as `labels`, and it returns the matched spans grouped by label.
...
Before the resume goes into a dashboard, an applicant-tracking system, or a shared review channel, the personal data has to come out. `ZeroGPURedactPIITool` masks PII inline on `gliner2.5-multi-v1`, replacing each match with an uppercase `[LABEL]` placeholder and returning the redacted text.
