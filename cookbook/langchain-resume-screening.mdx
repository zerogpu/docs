...
With the signal extracted and the PII gone, the last step is routing: which team should review this candidate? `ZeroGPUClassifyZeroShotTool` scores the redacted resume against a flat list of labels on `deberta-v3-small` and returns a score per label, so you can pick the best match. Routing on the anonymized copy keeps the whole downstream flow PII-free.
