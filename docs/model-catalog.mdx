...
{/*
  Curated cross-task grouping: the IAB classify models are surfaced as their
  own "Ad Tech" card in addition to their native Text Classification group.
*/}
export const AD_TECH_MODELS = [
  "zlm-v1-iab-classify-edge",
  "zlm-v1-iab-classify-edge-enriched",
];

{/*
  Card order follows the Models group in docs.json (Text Classification,
  Text Generation, PII, Summarization). The curated Ad Tech card has no
  docs.json page slot, so it leads as the featured grouping. Any task not
  listed here falls to the end in priority order.
*/}
...
export const ModelLibrary = () => {
  ...
  order.sort((a, b) => {
    ...
    return (ia === -1 ? TASK_ORDER.length : ia) - (ib === -1 ? TASK_ORDER.length : ib);
  });
  return (
    <div className="not-prose grid grid-cols-1 gap-4 md:grid-cols-2">
      {order.map((t) => (
