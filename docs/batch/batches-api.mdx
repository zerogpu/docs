...
See [Errors reference](/docs/batch/errors) for the auth failure codes (`401`, `403`, `429`).
...
| `401` / `403` / `429` / `500` | See [Errors reference](/docs/batch/errors). |
...
- `output_file_id` is set if at least one line returned 2xx. Download with
  ...
  [JSONL format](/docs/batch/jsonl-format#output-jsonl) for the line schema.
- `error_file_id` is set if at least one line failed. Download with
  `GET /v1/files/{error_file_id}/content`. See
  [JSONL format](/docs/batch/jsonl-format#error-jsonl) for the line schema.
