...
| `Line N url "X" is not an allowed batch endpoint` | `url` is not `/v1/chat/completions`, the only [supported batch endpoint](/api-reference/batch/supported-endpoints). |
...
<CardGroup cols={2}>
  <Card title="Batches API reference →" href="/docs/batch/batches-api">
    Status lifecycle, limits, full Batch object schema.
  </Card>
  <Card title="Files API reference →" href="/docs/batch/files-api">
