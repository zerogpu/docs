...
upload it via [`POST /v1/files`](/docs/batch/files-api#post-v1files--upload-a-file)
with `purpose=batch`, then reference its ID when you call
[`POST /v1/batches`](/docs/batch/batches-api#post-v1batches--create-a-batch).
...
| `body` | object | **Yes** | The request body that would be sent to the synchronous endpoint. See [Supported endpoints](/api-reference/batch/supported-endpoints) for the shape. The field `stream: true` is rejected. |
...
See [Supported endpoints](/api-reference/batch/supported-endpoints) for the
...
download it via
[`GET /v1/files/{file_id}/content`](/docs/batch/files-api#get-v1filesfile_idcontent--download-a-files-body).
...
See [Supported endpoints](/api-reference/batch/supported-endpoints) for the exact
...
<CardGroup cols={2}>
  ...
  <Card title="Examples →" href="/docs/batch/examples">
