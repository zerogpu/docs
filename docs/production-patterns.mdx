...
| `429` | Rate limit reached | Wait at least `Retry-After` seconds, then retry. See [Rate limits](#rate-limits). |
...
Counts reset at the start of each minute. Audio endpoints have request and concurrency limits, but no token limit.

Responses carry your limits in headers. A header is left out when that limit doesn't apply.

| Header | Example | Meaning |
|--------|---------|---------|
| `x-ratelimit-limit-requests` | `300` | Requests allowed per minute |
| `x-ratelimit-remaining-requests` | `295` | Requests left this minute |
| `x-ratelimit-reset-requests` | `37s` | Time until the request count resets |
| `x-ratelimit-limit-tokens` | `500000` | Tokens allowed per minute |
| `x-ratelimit-remaining-tokens` | `499000` | Tokens left this minute |
| `x-ratelimit-reset-tokens` | `37s` | Time until the token count resets |
| `x-ratelimit-limit-concurrent-requests` | `16` | Requests allowed in progress at once |
| `x-ratelimit-remaining-concurrent-requests` | `15` | Requests you can start right now |

A request over a limit is rejected with `429` before it runs, and `error.code` says which limit it hit:

```json
{
  "error": {
    "message": "Rate limit reached for deepseek-v4.1-flash in organization 3f2c... on requests per min (RPM): Limit 300, Used 300, Requested 1. Please try again in 37s.",
...
|--------------|---------------|------------|
| `requests_per_minute_exceeded` | Too many requests this minute | Wait for `Retry-After`, then retry. |
| `tokens_per_minute_exceeded` | Too many tokens this minute | Wait for `Retry-After`, then retry. If there is no `Retry-After`, this request alone is over your limit: reduce the input or `max_tokens`. |
| `concurrency_limit_exceeded` | Too many requests in progress | Retry after `Retry-After`, or when one of your requests finishes. |
