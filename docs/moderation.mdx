...
On `/v1/moderations` the response is OpenAI's moderations envelope, the same shape `zlm-v1-moderation-edge` returns. On the Responses and Chat Completions endpoints the verdict comes back as the output text — `safe`, or `unsafe` followed by the violated policy categories.
